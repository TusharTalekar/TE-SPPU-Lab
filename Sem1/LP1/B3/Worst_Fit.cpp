...
class WorstFit{
    ...
    public:
    WorstFit(int m=memorySize,int n=partitionNumber,vector<int> partSizes=partitionSizes){
        int remainSize=m;
        for(int i=0;i<n;i++){
...
int main(){
    ...
    // WorstFit w(size,n,partitionSize);
    WorstFit w;
    ...
    while (true){
        ...
        if(choice==1){
            ...
            cin>>size;
            addProcess(w.allocatePartition(size),memoryIndex);
        }else if(choice==4){
    ...
    // w.displayMemoryState();
    // addProcess(w.allocatePartition(21),memoryIndex);
    // addProcess(w.allocatePartition(19),memoryIndex);
    // addProcess(w.allocatePartition(25),memoryIndex);
    // addProcess(w.allocatePartition(31),memoryIndex);
    // addProcess(w.allocatePartition(4),memoryIndex);
    // addProcess(w.allocatePartition(25),memoryIndex);
    // displayMemoryIndex(memoryIndex);
    // w.displayMemoryState();
    // w.deallocate(memoryIndex[0]);
    // w.displayMemoryState();
