...
class BestFit{
    ...
    public:
    BestFit(int m=memorySize,int n=partitionNumber,vector<int> partSizes=partitionSizes){
        int remainSize=m;
        for(int i=0;i<n;i++){
...
int main(){
    ...
    // BestFit b(size,n,partitionSize);
    BestFit b;
    ...
    while (true){
        ...
        if(choice==1){
            ...
            cin>>size;
            addProcess(b.allocatePartition(size),memoryIndex);
        }else if(choice==4){
    ...
    // addProcess(b.allocatePartition(4),memoryIndex);
    // addProcess(b.allocatePartition(21),memoryIndex);
    // addProcess(b.allocatePartition(25),memoryIndex);
    // cout<<memoryIndex[1]<<endl;
    // displayMemoryIndex(memoryIndex);
    // b.displayMemoryState();
    // b.deallocate(memoryIndex[0]);
    // b.displayMemoryState();
