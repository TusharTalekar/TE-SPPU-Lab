...
class FirstFit{
    ...
    public:
    FirstFit(int m=memorySize,int n=partitionNumber,vector<int> partSizes=partitionSizes){
        int remainSize=m;
        for(int i=0;i<n;i++){
...
int main(){
    ...
    // FirstFit f(size,n,partitionSize);
    FirstFit f;
    ...
    while (true){
        ...
        if(choice==1){
            ...
            cin>>size;
            addProcess(f.allocatePartition(size),memoryIndex);
        }else if(choice==4){
    ...
    // f.displayMemoryState();
    // addProcess(f.allocatePartition(4),memoryIndex);
    // addProcess(f.allocatePartition(21),memoryIndex);
    // addProcess(f.allocatePartition(25),memoryIndex);
    // // cout<<memoryIndex[1]<<endl;
    // displayMemoryIndex(memoryIndex);
    // f.displayMemoryState();
    // f.deallocate(memoryIndex[0]);
    // f.displayMemoryState();
